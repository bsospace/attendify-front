...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const login = async (email: string, password: string) => {
    setIsLoading(true);
    try {
      const response = await apiClient.post(`${envConfig.apiUrl}${API_ENDPOINTS.AUTH.LOGIN}`, {
        email,
        password,
      });
  ...
  const logout = async () => {
    setIsLoading(true);
    try {
      await apiClient.get(`${envConfig.apiUrl}${API_ENDPOINTS.AUTH.LOGOUT}`);
      setUser(null);
  ...
  const getProfile = async () => {
    setIsLoading(true);
    try {
      const response = await apiClient.get(`${envConfig.apiUrl}${API_ENDPOINTS.AUTH.ME}`);
